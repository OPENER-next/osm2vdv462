...
def updatePathsElementsRef(cur, osmId, pathId, nodes=False, ways=False, areas=False):
    # only update (append to) the array of nodes, ways or areas of the given pathId, if the id is not already in the array
    # -- 'COALESCE(nodes, ARRAY[]::BIGINT[])' returns an empty array if the array is NULL, otherwise it returns the corresponding array
    # -- 'NOT (nodes @> ARRAY[%s])' checks if the array does not already contain the osmId
    # -- 'OR nodes IS NULL' checks if the array is NULL to compensate for the case where the database entry is NULL (@> does not work with NULL)
    if nodes:
        cur.execute(
            "UPDATE paths_elements_ref SET nodes = COALESCE(nodes, ARRAY[]::BIGINT[]) || %s WHERE path_id = %s AND (NOT (nodes @> ARRAY[%s]::BIGINT[]) OR nodes IS NULL)",
            (osmId, pathId, osmId)
        )
    if ways:
        cur.execute(
            "UPDATE paths_elements_ref SET ways = COALESCE(ways, ARRAY[]::INTEGER[]) || %s WHERE path_id = %s AND (NOT (ways @> ARRAY[%s]) OR ways IS NULL)",
            (osmId, pathId, osmId)
        )
    if areas:
        cur.execute(
            "UPDATE paths_elements_ref SET areas = COALESCE(areas, ARRAY[]::INTEGER[]) || %s WHERE path_id = %s AND (NOT (areas @> ARRAY[%s]) OR areas IS NULL)",
            (osmId, pathId, osmId)
        )
...
def insertPathsElementsRef(cur, pathId, edges):
    ...
    # first check, if there is an entry for the pathId in the database
    cur.execute(
        "SELECT COUNT(*) FROM paths_elements_ref WHERE path_id = %s",
        (pathId,)
    )
    if cur.fetchone()[0] == 0:
        # insert a new entry for the pathId
        cur.execute(
            "INSERT INTO paths_elements_ref (path_id) VALUES (%s)",
            (pathId,)
        )
    
    for edge in edges:
        if edge["edge_type"] == "footway":
            if edge["osm_way_id"] < 0:
                # path is going trough an area ("osm_way_id" = negative value of the way id)
                updatePathsElementsRef(cur, abs(edge["osm_way_id"]), pathId, areas=True)
            elif edge["osm_way_id"] > 0:
                # path is a way
                updatePathsElementsRef(cur, edge["osm_way_id"], pathId, ways=True)
            else:
                continue # skip edges with osm_way_id == 0
        elif edge["edge_type"] == "street":
            # insert the osm way id of the street
            if edge["osm_way_id"] < 0:
                # path is going trough an area ("osm_way_id" = negative value of the way id)
                print("WARNING: Street is going trough an area!")
            updatePathsElementsRef(cur, edge["osm_way_id"], pathId, ways=True)
        elif edge["edge_type"] == "crossing":
            ...
            if edge["crossing_type"] == "generated":
                ...
                if edge["from_node_osm_id"] != 0:
                    updatePathsElementsRef(cur, edge["from_node_osm_id"], pathId, nodes=True)
                else:
                    updatePathsElementsRef(cur, edge["to_node_osm_id"], pathId, nodes=True)
                updatePathsElementsRef(cur, abs(edge["osm_way_id"]), pathId, ways=True)
            else:
...
def insertPathLink(cur, pathLink, id_from, id_to):
    ...
    if path_id:
        return path_id[0] # return the path_id
    else:
        return None
